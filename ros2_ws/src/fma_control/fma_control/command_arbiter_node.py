...
def select_source(
        lane, mission, emergency_latched, now,
        lane_timeout, mission_timeout,
        manual=None, manual_timeout=0.5,
        gps=None, gps_timeout=0.5,
        control_mode=None):
    ...
    # Mission safety or mission controller outranks autonomous road following.
    if candidate_is_valid(mission) and 0 <= now - mission.received_at < mission_timeout:
        return 'mission'

    # GPS mode must never silently fall back to lane control.
    if control_mode == 'GPS':
        if candidate_is_valid(gps) and 0 <= now - gps.received_at < gps_timeout:
            return 'gps'
        return None

    # Preserve legacy/startup lane behaviour until the first mode arrives.
    if control_mode in (None, 'LANE'):
        if candidate_is_valid(lane) and 0 <= now - lane.received_at < lane_timeout:
            return 'lane'
...
class CommandArbiterNode(Node):
    ...
    def receive_candidate(self, source, msg):
        ...
        rejected_pwm = source in ('lane', 'mission', 'gps') and msg.pwm_control and not raw_pwm_allowed
