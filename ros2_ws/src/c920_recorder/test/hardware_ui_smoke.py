...
Not collected by pytest. Creates a new 10-second recording; does not launch camera.
"""
import argparse
import json
import time
...
from c920_recorder.plugin import RecorderPlugin
...
def main():
    ...
    app = QApplication([])
    rclpy.init()
    node = rclpy.create_node('c920_ui_smoke')
    ...
    opened=[]
    opener=patch.object(QDesktopServices,'openUrl',side_effect=lambda url: opened.append(url.toLocalFile()) or True)
    opener.start()
    plugin.autoplay.setChecked(True)
    def pump():
    ...
    try:
        ...
        assert opened==[str(plugin.recorder.path/'color.mp4')], opened
        plugin.play_button.click();plugin.folder_button.click()
        assert opened[-2:]==[str(plugin.recorder.path/'color.mp4'),str(plugin.recorder.path)]
