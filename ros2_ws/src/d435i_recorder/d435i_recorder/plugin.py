...
class RecorderPlugin(Plugin):
    def __init__(self, context):
        ...
        self.setObjectName('D435iRecorder')
        # Keep recorder callbacks out of rqt's shared MultiThreadedExecutor.
        self.ros_node = rclpy.create_node('d435i_recorder_' + str(id(self)))
        self.executor = SingleThreadedExecutor()
        self.executor.add_node(self.ros_node)
        self.recorder = Recorder(self.ros_node)
        self.spin_stop = threading.Event()
        def spin():
            while not self.spin_stop.is_set() and rclpy.ok():
                self.executor.spin_once(timeout_sec=.05)
        self.spinner = threading.Thread(target=spin, name='d435i-ros')
        self.spinner.start()
        self.widget = QWidget()
        self.widget.setWindowTitle('D435i Recorder')
        ...
        layout.addWidget(QLabel('흰색 정지선 후보 baseline · 거리 = optical Z · 검출 정확도 미검증'))
    ...
    def update(self):
        ...
        if rec.preview is not None:
            ...
            boxes = ([r['bbox'] for r in rec.writer.latest_results if r.get('bbox')]
                     if rec.writer else candidates(frame))
        ...
        if rec.finishing:
            ...
            text = f'RECORDING: {dict(rec.writer.counts)}'
        elif rec.result:
            ...
            try:
                ...
                self.timer.stop()
                return
        self.status.setText(self.ui_error or text)
